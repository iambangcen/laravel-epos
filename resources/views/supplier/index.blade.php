...
@section('bot')
    ...
    <script type="text/javascript">
        ...
        function viewdata(id) {
           window.location.href = "{{url('lihatSupplier')}}" + "/" + id; // maybe you can discard the slash after room_detail.
        }

        function addForm() {
            save_method = 'add';
            $('input[name=_method]').val('POST');
            $('#modal-form-sup').modal('show');
            $('#modal-form-sup form')[0].reset();
            $('.modal-title').text('Form Tambah Supplier');
        }
        ...
        $(function(){
            $('#modal-form-sup form').validator().on('submit', function (e) {
...
//                      data : $('#modal-form form').serialize(),
                        data: new FormData($("#modal-form-sup form")[0]),
                        contentType: false,
                        processData: false,
                        success : function(data) {
                            $('#modal-form-sup').modal('hide');
                            table.ajax.reload();
                            swal({
                                title: 'Success!',
                                text: data.message,
